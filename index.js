...
client.on("message", async message => {
    ...
    // clear queue and disconnect
    else if (message.content.startsWith(`${prefix}odjebi`)) { // clear queue and disconnect
        ...
        if (!serverQueue) {
            ...
            try {
                ...
                // embed.fields[0] = "Songs in the queue:", " ";
                let i = 1;
        ...
        message.channel.send(
            ...
            .addFields({ name: "**!play**", value: "Plays a song from youtube or playlist from youtube. Valid arguments are keywords, direct links or playlist links" }, { name: "**!stop**", value: "Stops the currently playing song, clears the queue, leaves the channel" }, { name: "**!skip**", value: "Skips a song" }, { name: "**!queue**", value: "Displays the queue" }, { name: "**!clearqueue**", value: "Clears the queue" }, { name: "**!odjebi**", value: "Stops everything, clears everything, a reboot switch pretty much" })
        );
    } else {
...
async function execute(message, serverQueue) {
    ...
    if (!isPlaylist) { //if ytpl didn't return a playlist ID it's either a youtube URL or a keyword search
        try { //figure out if URL or keyword search
    ...
    if (song) {
        ...
        const dispatcher = serverQueue.connection
            ...
            .on("finish", () => {
                ...
                //console.log("Finished: " + `${song.title}` + " URL: " + ` ${song.url}`); //debuf info
                play(guild, serverQueue.songs[0]);
            })
            .on("error", error => console.error(error));
        dispatcher.setVolumeLogarithmic(serverQueue.volume / 5);
        //console.log("Playing: ".green + `${song.title}` + " URL: ".green + `${song.url}`); //debuf info
